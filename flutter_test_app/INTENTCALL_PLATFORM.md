...
Normal repo state resolves hosted `intentcall_* ^0.1.0` packages from pub.dev. Local path overrides to `/Users/anton/mcp/agentkit/packages/intentcall_*` are local-development-only and should not be committed for hosted consumer integration.
...
| Path | Consumer proof |
|------|----------------|
| WebMCP | `web/index.html` loads generated WebMCP JS for early discovery; Flutter bootstrap installs the Dart execute hook for app entries. |
...
| Native invoke | `IntentCallInvokeLinkListener` in `lib/main.dart` logs `intentcall://invoke/<qualifiedName>` deep links. |
