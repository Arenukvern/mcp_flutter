...
void main() {
  ...
  testWidgets('press_key Ctrl+S dispatches a Shortcuts/Intent/Action handler', (
    final tester,
  ) async {
    ...
    final result = await ControlFlowService.pressKey(key: 's', ctrl: true);
    await tester.pump();
  ...
  testWidgets('navigate pop returns to previous route', (final tester) async {
    ...
    unawaited(navKey.currentState!.pushNamed('/inner'));
    await tester.pumpAndSettle();
  ...
  testWidgets('hover returns ref_not_found for an unknown ref', (
    ...
    final result = await GestureInteractionService.hoverAtRef(
      's_does_not_exist',
    );
    expect(result['success'], isFalse);
