...
void main() {
  ...
  setUp(() {
    tmp = Directory.systemTemp.createTempSync('init_agentkit_platform_');
  ...
  test('init agentkit-platform applies hooks', () async {
    expect(await runInitAgentkitPlatform(projectRoot: tmp.path), 0);
    final html = File(p.join(tmp.path, 'web', 'index.html')).readAsStringSync();
    expect(html, contains('agentkit-platform: begin'));
  });

  test('--check fails before apply', () async {
    expect(await runInitAgentkitPlatform(projectRoot: tmp.path, checkOnly: true), isNot(0));
  });
}
