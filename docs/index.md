...
>
> The **main goal of this project** is to bring power of MCP server tools by creating them in Flutter app, using **dynamic MCP tools registration** . See how it works in [short YouTube video](https://www.youtube.com/watch?v=Qog3x2VcO98). See [Quick Start](getting-started/quick-start.md) for more details. See [original motivation](https://github.com/Arenukvern/mcp_flutter/blob/main/CHANGELOG.md#210) behind the idea.
>
> Also, secondary goal is to stabilize and polish tools which are useful in development (so it would be specifically targeted for AI Assistants, for example: it will return not only the errors, but prompt for AI how to work with that error) [see more in MCP_RPC_DESCRIPTION.md](architecture/mcp-rpc-description.md)
>
...
See more details in [Changelog](changelog.md).
...
See more details about command line options in [MCP Server Dart README](mcp-server/readme.md).
...
- Quick Start is available in [QUICK_START.md](getting-started/quick-start.md)
- Configuration options are available in [CONFIGURATION.md](getting-started/configuration.md)
...
📚 Please see more in [MCP_RPC_DESCRIPTION](architecture/mcp-rpc-description.md)

## 🔒 Security

Generally, since you use MCP server to connect to Flutter app in Debug Mode, it should be safe to use. However, I still recommend to review how it works in [Architecture](architecture/overview.md), how it can be modified to improve security if needed.
