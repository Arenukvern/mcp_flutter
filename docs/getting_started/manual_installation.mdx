...
Startup is non-blocking: the server can initialize without locking to a specific VM target. Target selection is resolved when the first VM-dependent request is executed.

If multiple debug targets are discovered and none is already sticky/current, VM-dependent calls return `connection_selection_required` with available targets and retry guidance (`connection.targetId`).
...
Follow the instructions in the [Manual Client Setup](/getting_started/manual_client_setup) guide to connect your AI assistant to the running MCP server.
