...
### Multiple Debug Targets (Important)

When multiple Flutter debug apps are running:

- VM-dependent tools do not guess the target.
- The server returns `connection_selection_required` with `availableTargets` and a retry example.
- Retry the same tool call with `arguments.connection.targetId`, or call `connect_debug_app` with the same `connection` object.
- The same nested `connection` object is also used by CLI one-shot `exec --args`, daemon `command/execute` / `watch/start`, and snapshot step args.
