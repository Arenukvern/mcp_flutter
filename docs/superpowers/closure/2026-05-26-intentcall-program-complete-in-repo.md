# intentcall program — complete in-repo (Phase 6 closure)

> **Historical snapshot** (Bar D, 2026-05-26). “Deferred → Phase 7” items for init/plugin/
> `fmt_migrate` were shipped in [product closure](./2026-05-26-intentcall-product-complete-in-repo.md).
...
`MCPCallEntry` hard cut, server `@AgentTool` codegen pilot, `intentcall_platform` emitters
...
| 6d-web | `f3614b0` Web C bootstrap + `intentcall_platform` | done |
...
| `dart test packages/intentcall_platform` | pass (17) |
...
Full matrix (`dart test packages/intentcall_*`, `cd mcp_server_dart && dart test`) should run in CI before merge to `main`.

## Delivered (6g / 6h)

- Skill **`flutter-mcp-toolkit-intentcall-migration`** (CLI, before/after patterns)
...
- Grep contract **`tool/contracts/check_intentcall_skills_grep.sh`**

## Deferred → Phase 7 (at this gate)

- Standalone `intentcall` monorepo + pub.dev publish
...
Follow-up integration pass: [2026-05-26-intentcall-integration-complete.md](./2026-05-26-intentcall-integration-complete.md).  
...
Next: Phase 7 extract plan from [intentcall design spec](../specs/2026-05-25-intentcall-design.md).
