...
# Dogfood vs agentkit spec — gap matrix

**Date:** 2026-05-26  
**Spec:** [2026-05-25-agentkit-design.md](../../specs/2026-05-25-agentkit-design.md), Bar D [pre-extract completion](../../specs/2026-05-26-agentkit-pre-extract-completion-design.md)  
**Evidence:** [`.showcase/dogfood_web_eval.yaml`](../../../.showcase/dogfood_web_eval.yaml) (iterations 1–10), code + CI on `feat/agentkit-phase1-3`
...
|------------------|----------------|----------|--------------|
| Web WebMCP (`navigator.modelContext.registerTool`, JS + Dart bootstrap) | Shipped (with Chrome flags) | `agent_web_mcp_bootstrap_web.dart`, `agentkit_webmcp.generated.js`, `make web-showcase`, `webmcp verify`; iter **7** `webmcp_verdict: webmcp_active` | **low** — plain `flutter run -d chrome` omits flags |
| WebMCP registry hot-sync (`WebMcpPublishAdapter`) | Package + tests; not in dogfood app | `agentkit/packages/agentkit_webmcp`; eval note: dogfood uses `AgentWebMcpBootstrap` | **low** |
...
| Dynamic listing name vs qualified descriptor | Documented asymmetry | `AGENTKIT_PLATFORM.md`; tracker `fix_recommendations` | **low** |
| `init agentkit-platform` + `codegen sync` | Shipped | CLI + tests; `make check-contracts`; dogfood static battery | **none** locally |
| CI drift gates on every PR | Partial | Gating: `agentkit_eval.yml` → `dogfood-eval-static`; gap: full `check-contracts`, skills grep | **high** |
...
| CLI exec vs MCP `fmt_*` names | Shipped | `CommandCatalog.resolveExecCommandName`; iter **3**; `AGENTKIT_PLATFORM.md` | **none** |
...
| Theme | Goal |
|-------|------|
| I11 — CI truth | Promote `check_agentkit_skills_grep.sh` + fail-hard contract steps on PR |
...
- [Integration completion plan](../../plans/archive/2026-05-26-agentkit-integration-completion-next.md)
- [WebMCP verification](../2026-05-26-webmcp-verification.md)
- [tool_quality_rubric.yaml](../tool_quality_rubric.yaml)
- [agentkit-rollout.yaml](../../tracker/agentkit-rollout.yaml)
