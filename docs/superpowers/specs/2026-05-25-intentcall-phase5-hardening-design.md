# intentcall Phase 5 — Hardening Design
...
- Align [rollout](../plans/2026-05-25-intentcall-rollout.md) phase table with tracker (phases 1–4 `done`, Phase 5 row).
...
- Note Phase 1 codegen: **annotations-only** (`intentcall_codegen`); no `build_runner` generator yet.
...
- `intentcall_codegen`: optional `build_runner` generator for `@AgentTool` → `RegisteredAgentIntent` (pilot one package).
...
- [x] `AgentToolGenerator` + `build.yaml` in `intentcall_codegen`
...
- [x] `intentcall_apple` / `intentcall_android` README author workflows
...
| Item | Rationale |
|------|-----------|
| Standalone **intentcall** monorepo split | Consumer workspace milestone sufficient; extract when release/versioning needs it |
| **Public shim** removal | Deprecation window; breaking importers without semver bump |
| **Gemma product wiring** | `intentcall_gemma` ships example-only registrar callbacks; `flutter_gemma` stays app-owned |
...
| Phase 5-C codegen | Pilot in `intentcall_codegen` test fixture | Server/flutter_test_app codegen deferred; hand-written path first-class |
...
- Design spec: [2026-05-25-intentcall-design.md](2026-05-25-intentcall-design.md)
- Phase 4 closure: [../closure/2026-05-25-intentcall-phase4-registry-resources.md](../closure/2026-05-25-intentcall-phase4-registry-resources.md)
- Self-closing loop: [../intentcall-self-closing-loop.md](../intentcall-self-closing-loop.md)
