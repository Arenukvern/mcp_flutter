# intentcall Self-Closing Implementation Loop

> **Purpose:** After each phase, a dedicated **Closer** agent verifies the repo against the design spec, regenerates the **current** phase plan if gaps remain, or authors the **next** phase plan when exit criteria pass. Repeat until the full [intentcall design spec](specs/2026-05-25-intentcall-design.md) is implemented.

**Rollout tracker:** [plans/2026-05-25-intentcall-rollout.md](plans/2026-05-25-intentcall-rollout.md)  
**Program state (YAML):** [tracker/intentcall-rollout.yaml](tracker/intentcall-rollout.yaml)
...
| Phase 7 extract | Standalone monorepo + pub.dev — **in progress** (7.1–7.3, 7.6 done; 7.4–7.5, 7.7 pending — see [tracker](tracker/intentcall-rollout.yaml)) |
...
**Path:** `docs/superpowers/closure/YYYY-MM-DD-intentcall-<phase-id>.md`

**Template:**

```markdown
# Closure: intentcall <phase-id>
...
- [ ] Regenerated `plans/2026-05-25-intentcall-<phase>.md` (same phase, vN+1)
- [ ] Regenerated `plans/2026-05-25-intentcall-phase<N+1>.md` (next phase)
- [ ] Updated `tracker/intentcall-rollout.yaml`
```
...
1. Set `program.status: complete` in `tracker/intentcall-rollout.yaml`.
2. Write `docs/superpowers/closure/YYYY-MM-DD-intentcall-program-complete.md`.
...
```text
Implement intentcall <phase-id> using docs/superpowers/plans/<phase-plan>.md.
...
```text
You are the Closer for intentcall <phase-id>. Do not implement features.
...
3. Write docs/superpowers/closure/YYYY-MM-DD-intentcall-<phase-id>.md.
4. If fail: regenerate same-phase plan (v+1) with Fix tasks only for gaps.
5. If pass: update docs/superpowers/tracker/intentcall-rollout.yaml; regenerate next phase plan; set active_phase to next.
...
| Artifact | Path |
|----------|------|
| Design spec (source of truth) | `docs/superpowers/specs/2026-05-25-intentcall-design.md` |
| Active phase plan | `docs/superpowers/plans/2026-05-25-intentcall-phase<N>.md` |
| Rollout overview | `docs/superpowers/plans/2026-05-25-intentcall-rollout.md` |
| Machine tracker | `docs/superpowers/tracker/intentcall-rollout.yaml` |
| Closure reports | `docs/superpowers/closure/` |
| This loop | `docs/superpowers/intentcall-self-closing-loop.md` |

**AGENTS.md / CLAUDE.md:** Point intentcall work at this loop before starting implementation.
