# intentcall Phase 1 Implementation Plan
...
**Goal:** Introduce transport-agnostic `intentcall_schema` + `intentcall_core` inside the mcp_flutter workspace, route all static MCP tool invocations through `AgentRegistry.invoke`, and add hand-written + optional-codegen authoring paths without breaking external MCP/CLI contracts.

**Architecture:** New packages sit under `packages/intentcall_*`. `McpHost` dual-registers each `ToolRegistration` into `InMemoryAgentRegistry` and keeps existing `dart_mcp` publish bridge until Phase 2. Handlers become thin wrappers: `CallToolRequest` → `registry.invoke` → `AgentResult` → `CallToolResult`. `CoreCommand` stays for VM/session orchestration; tool-shaped work maps to intents.

**Tech Stack:** Dart 3.11 workspace, `meta`, `test`, existing `dart_mcp ^0.5.0`, optional `build_runner` + `source_gen` for codegen pilot.

**Design spec:** `docs/superpowers/specs/2026-05-25-intentcall-design.md`
...
>
> **Self-closing loop:** Implementer completes tasks → **Closer** runs [Phase Gate](../intentcall-self-closing-loop.md#phase-gate-checklist-closer) (Task 13). Do not advance to Phase 2 until tracker marks `phase1: done`. Program: [rollout](2026-05-25-intentcall-rollout.md).
...
|----------------|----------------|
| `packages/intentcall_schema/` | `AgentResult`, `envelope`, `AgentWireArgs`, `InputSchema`, validation |
| `packages/intentcall_core/` | `AgentIntentDescriptor`, `RegisteredAgentIntent`, `AgentRegistry`, `AgentRuntime`, `AgentAdapter`, `AgentCallEntry` |
| `packages/intentcall_codegen/` | `@AgentTool`, `@AgentParam`, generator pilot |
| `packages/intentcall_testing/` | `FakeAgentAdapter`, registry contract helpers |
...
| `packages/intentcall_core/lib/src/module/agent_module_from_entries.dart` | `AgentModule.fromEntries` builder pattern |
...
**Explicitly not in Phase 1:** `intentcall_webmcp`, `intentcall_gemma`, `intentcall_apple`, removing `dart_mcp` from kernel, `DynamicRegistry` rewrite.

---

## Task 1: Workspace — add `intentcall_schema` package

**Files:**
- Create: `packages/intentcall_schema/pubspec.yaml`
- Create: `packages/intentcall_schema/analysis_options.yaml`
- Create: `packages/intentcall_schema/lib/intentcall_schema.dart`
- Create: `packages/intentcall_schema/lib/src/agent_result.dart`
- Create: `packages/intentcall_schema/lib/src/input_schema.dart`
- Create: `packages/intentcall_schema/lib/src/schema_validator.dart`
- Create: `packages/intentcall_schema/test/schema_validator_test.dart`
...
```bash
cd /Users/anton/mcp/mcp_flutter/packages/intentcall_schema
...
```bash
git add packages/intentcall_schema pubspec.yaml
git commit -m "feat(intentcall): add intentcall_schema package"
```

---

## Task 2: `intentcall_core` — descriptors, registrations, naming

**Files:**
- Create: `packages/intentcall_core/pubspec.yaml`
- Create: `packages/intentcall_core/lib/intentcall_core.dart`
- Create: `packages/intentcall_core/lib/src/intent/agent_intent.dart`
- Create: `packages/intentcall_core/lib/src/intent/callable_agent_intent.dart`
- Create: `packages/intentcall_core/lib/src/naming/qualified_name.dart`
- Create: `packages/intentcall_core/lib/src/adapter/agent_adapter.dart`
- Create: `packages/intentcall_core/lib/src/module/agent_module.dart`
- Create: `packages/intentcall_core/test/qualified_name_test.dart`
- Modify: root `pubspec.yaml` workspace

- [ ] **Step 1: Add workspace member `packages/intentcall_core`**

`pubspec.yaml` depends on `intentcall_schema` only (no `dart_mcp`).
...
```bash
cd packages/intentcall_core && dart test && cd ../..
git add packages/intentcall_core pubspec.yaml
git commit -m "feat(intentcall): add descriptor, RegisteredAgentIntent, naming"
```
...
**Files:**
- Create: `packages/intentcall_core/lib/src/registry/agent_registry.dart`
- Create: `packages/intentcall_core/lib/src/registry/in_memory_agent_registry.dart`
- Create: `packages/intentcall_core/lib/src/registry/registry_events.dart`
- Create: `packages/intentcall_core/test/in_memory_agent_registry_test.dart`
...
```bash
cd packages/intentcall_core && dart test
git commit -m "feat(intentcall): add InMemoryAgentRegistry"
```

---

## Task 1b: Client DX helpers (`intentcall_schema` + `mcp_toolkit`)

**Files:**
- Create: `packages/intentcall_schema/lib/src/agent_result_envelope.dart`
- Create: `packages/intentcall_schema/lib/src/agent_wire_args.dart`
- Create: `packages/intentcall_core/lib/src/module/agent_module_from_entries.dart`
- Create: `mcp_toolkit/lib/src/agent_client_install.dart`
- Create: `packages/intentcall_schema/test/agent_wire_args_test.dart`
- Create: `packages/intentcall_schema/test/agent_result_envelope_test.dart`
...
```bash
git commit -m "feat(intentcall): add ecsly client DX helpers (envelope, wire, lazy install)"
```
...
**Files:**
- Create: `packages/intentcall_core/lib/src/authoring/agent_call_entry.dart`
- Create: `packages/intentcall_core/test/agent_call_entry_test.dart`
...
```bash
git commit -m "feat(intentcall): add AgentCallEntry authoring"
```
...
**Files:**
- Create: `packages/intentcall_core/lib/src/runtime/agent_runtime.dart`
- Create: `packages/intentcall_core/test/agent_runtime_test.dart`
...
- Modify: `packages/server_capability_kernel/pubspec.yaml` — add `intentcall_core`, `intentcall_schema`
...
- Modify: `mcp_server_dart/pubspec.yaml` — `intentcall_core`, `intentcall_schema`
...
## Task 8: `intentcall_testing` + MCP parity contract

**Files:**
- Create: `packages/intentcall_testing/pubspec.yaml`
- Create: `packages/intentcall_testing/lib/intentcall_testing.dart`
- Create: `packages/intentcall_testing/lib/src/registry_contract.dart`
...
**Files:**
- Create: `mcp_toolkit/lib/src/agent_call_entry.dart` (re-export from intentcall_core or thin wrapper)
- Modify: `mcp_toolkit/pubspec.yaml` — `intentcall_core`
...
@Deprecated('Use AgentCallEntry from intentcall_core')
...
## Task 10: `intentcall_codegen` pilot (optional server + client)

**Files:**
- Create: `packages/intentcall_codegen/pubspec.yaml`
- Create: `packages/intentcall_codegen/lib/intentcall_codegen.dart`
- Create: `packages/intentcall_codegen/lib/src/annotations.dart`
- Create: `packages/intentcall_codegen/lib/src/agent_tool_generator.dart`
...
```bash
git commit -m "feat(intentcall): add optional @AgentTool codegen pilot"
```
...
**Files:**
- Modify: `docs/superpowers/specs/2026-05-25-intentcall-design.md` — status → Approved
- Create: `packages/intentcall_core/README.md`
- Modify: `docs/guides/creating_dynamic_tools.mdx` — mention `AgentCallEntry` + optional codegen

- [ ] **Step 1: README for intentcall_core** — authoring table (hand-written vs codegen, server vs client)
...
```bash
git commit -m "docs: intentcall Phase 1 authoring and architecture"
```
...
dart test packages/intentcall_schema
...
**Files:**
- Create: `docs/superpowers/closure/YYYY-MM-DD-intentcall-phase1.md`
- Modify: `docs/superpowers/tracker/intentcall-rollout.yaml`
- Modify (on fail): this plan file — bump `Plan revision` to v2+ with **Fix:** tasks only
- Create (on pass): `docs/superpowers/plans/2026-05-25-intentcall-phase2.md` via writing-plans

- [ ] **Step 1: Run ALL validation commands** from [tracker](../tracker/intentcall-rollout.yaml) `phases[phase1].validation` plus Task 12 commands. Fresh run; capture exit codes.

- [ ] **Step 2: Spec coverage audit** — fill checklist below with `pass` | `fail` | `partial` and file evidence per row.

- [ ] **Step 3: Write closure report** at `docs/superpowers/closure/YYYY-MM-DD-intentcall-phase1.md` with verdict `pass` or `fail`.
...
- [ ] **Step 4b: If `pass`** — Update tracker: `phase1.status: done`, `phase1.last_gate: pass`, `program.active_phase: phase2`. Generate [phase2 plan](2026-05-25-intentcall-phase2.md) from design spec Phase 2 using writing-plans. Commit tracker + closure + phase2 plan.
...
```text
You are the Closer for intentcall phase1. Do not implement features.
Follow docs/superpowers/intentcall-self-closing-loop.md Task 13 and Phase Gate checklist.
...
|------------------|------|--------|----------|
| `intentcall_schema` | Task 1 |
| `intentcall_core` registry/runtime | Tasks 2–5 |
...
- Extract `intentcall_mcp` package; delete `DartMcpDispatchBridge` from host
...
- In-repo path deps until separate intentcall repo (Phase 3)
