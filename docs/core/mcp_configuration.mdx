...
| `--runner-session-file` | Path to an external dev-session contract file (`Dart dev session contract`, spec v2). Default: `<project>/.flutter_mcp/runner-session.json`. When present and alive, reload/restart delegate to the owning session (the only compile-capable channel) and machine discovery is suppressed. See [ADR 0014](/decisions/0014_external_dev_sessions_runner_delegation). |
...
### Delegated reload / restart (external dev session)

Some runners own the app session **and** the compiler — for example
[`oka dev`](https://github.com/Arenukvern/oka), which drives
`flutter attach --machine` and is the only compile-capable channel while it
runs. When a runner-session file is present (default
`.flutter_mcp/runner-session.json`), the toolkit treats that session as the
owner:

- `hot_reload_flutter` / `hot_restart_flutter` **delegate** to the owning
  session over its control port; the result carries the actual daemon
  outcome plus `changed` (whether a compiled delta was applied) and
  `delegated: true` with `runner` metadata from the file.
- Machine discovery is suppressed — a second attach session would kill the
  first.
- VM-dependent **reads** (snapshots, screenshots, evaluate) still use the
  normal connection machinery; pass the file's `vm_service_uri` via
  `connection` (`mode: "uri"`) when discovery is ambiguous.
- When no runner-session file exists, behavior is unchanged: the toolkit
  connects to VM services directly, and reload/restart results carry an
  honest `changed: false` when no compile-capable channel was found (late
  VM-service attaches cannot compile — see
  [ADR 0014](/decisions/0014_external_dev_sessions_runner_delegation)).
