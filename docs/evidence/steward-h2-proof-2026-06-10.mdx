...
- `AGENTS.md` records the portable `tool/steward/run.sh` wrapper and local Skill Steward checkout override for environments where global `steward` is stale.

## Portable commands

Run from the repository root. The wrapper uses a compatible `steward` on `PATH`, or `SKILL_STEWARD_ROOT=<skill-steward-checkout>` for local Skill Steward development.
...
bash tool/steward/run.sh benchmark --scenario mcp_flutter.web-dogfood-warm --strict --output .steward/benchmark-summaries/mcp_flutter.web-dogfood-warm.strict.json --json
```

## Local provenance

The original run used the maintainer's private Dart SDK path plus a sibling Skill Steward checkout. That raw `dart --packages=... steward.dart` form is non-copyable provenance only; future evidence should use the wrapper command above and record `SKILL_STEWARD_ROOT` only as a local override.
