// mcp_capability_core/test/tools/inspection_tools_test.dart
import 'dart:convert';

import 'package:dart_mcp/server.dart';
import 'package:mcp_capability_core/src/tools/inspection_tools.dart';
...
void main() {
  ...
  group('inspection tools — get_view_details', () {
    ...
    test('get_view_details schema: additionalProperties false, no required', () {
      final ctx = _registeredCtx();
    ...
    test('get_view_details handler short-circuits on override failure',
        () async {
      final runner = FakeCommandRunner()
  ...
  group('inspection tools — inspect_widget_at_point', () {
    ...
    test('inspect_widget_at_point schema: required [x, y], x/y/viewId integer',
        () {
      final ctx = _registeredCtx();
      final schema =
    ...
    test(
      ...
      final runner = FakeCommandRunner()
        ..nextExecuteResult =
            CoreResult.success(data: {'widget': 'Text'});
      final ctx = _registeredCtx(runner: runner);
    ...
    test('inspect_widget_at_point handler short-circuits on override failure',
        () async {
      final runner = FakeCommandRunner()
        ..nextOverrideResult = CoreResult.failure(
          code: CoreErrorCode.connectFailed,
  ...
  group('inspection tools — get_app_errors', () {
    ...
    test('get_app_errors handler fans out message + per-error TextContent',
        () async {
      final runner = FakeCommandRunner()
        ..nextExecuteResult = CoreResult.success(
          data: {
    ...
    test('get_app_errors with no errors returns single message content',
        () async {
      final runner = FakeCommandRunner()
        ..nextExecuteResult = CoreResult.success(
          data: {'message': 'No errors found', 'errors': []},
