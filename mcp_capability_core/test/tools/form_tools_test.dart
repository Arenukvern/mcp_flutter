...
void main() {
  ...
  group('form tools — fill_form registration', () {
    ...
    test('fill_form schema — fields is array type', () {
      final ctx = _registeredCtx();
      final props = ctx.registrationFor('fill_form')!.inputSchema['properties']
          as Map<String, Object?>;
      final fieldsSchema = props['fields'] as Map<String, Object?>;
      expect(fieldsSchema['type'], equals('array'));
    });

    test('fill_form schema — fields items have additionalProperties:false and required [ref, text]',
        () {
      final ctx = _registeredCtx();
      final props = ctx.registrationFor('fill_form')!.inputSchema['properties']
          as Map<String, Object?>;
      ...
      expect((itemProps['ref'] as Map<String, Object?>)['type'], equals('string'));
      expect(
  ...
  group('form tools — fill_form handler command construction', () {
    test('handler builds FillFormCommand with fields list and snapshotId',
        () async {
      final fakeRunner = FakeCommandRunner();
      final ctx = _registeredCtx(runner: fakeRunner);
      await ctx.registrationFor('fill_form')!.handler(
        CallToolRequest(
          name: 'fill_form',
    ...
    test('handler uses empty fields list when fields arg is not a List',
        () async {
      final fakeRunner = FakeCommandRunner();
      final ctx = _registeredCtx(runner: fakeRunner);
    ...
    test('handler calls applyConnectionOverride before execute', () async {
      ...
      await ctx.registrationFor('fill_form')!.handler(
        CallToolRequest(name: 'fill_form', arguments: args),
      );
