...
**Architecture:** The tool prefix comes from `CoreCapability.id` (`flutter_mcp_toolkit_capability_core/lib/src/core_capability.dart`). The kernel constructs MCP tool names as `<capability_id>_<tool_name>`. Renaming `id` from `'core'` to `'fmt'` flips the prefix everywhere atomically. Binary renames are `makefile` + `install.sh` checksum updates.

**Tech Stack:** Dart, Make, shell.

**Spec reference:** `specs/2026-04-29-flutter-mcp-toolkit-plugin-design.md` §5, §6.1.
...
**Modify:**
- `flutter_mcp_toolkit_capability_core/lib/src/core_capability.dart` — id `'core'` → `'fmt'`; consider class rename
...
- `flutter_mcp_toolkit_capability_core/lib/flutter_mcp_toolkit_capability_core.dart` — export rename if class renamed
...
Run: `grep -n "id =>" flutter_mcp_toolkit_capability_core/lib/src/core_capability.dart`
...
**Files:**
- Modify: `flutter_mcp_toolkit_capability_core/lib/src/core_capability.dart`

- [ ] **Step 4.1: Update the id getter**

Edit `flutter_mcp_toolkit_capability_core/lib/src/core_capability.dart`:
...
```bash
git add flutter_mcp_toolkit_capability_core/lib/src/core_capability.dart
...
**Files:**
- Modify: `flutter_mcp_toolkit_capability_core/lib/src/core_capability.dart` (or rename to `fmt_capability.dart`)
- Modify: `flutter_mcp_toolkit_capability_core/lib/flutter_mcp_toolkit_capability_core.dart` — update export
...
Run: `grep -rn "CoreCapability" mcp_server_dart/ flutter_mcp_toolkit_capability_core/lib/`

- [ ] **Step 5.3: Rename file**

```bash
git mv flutter_mcp_toolkit_capability_core/lib/src/core_capability.dart \
...
Run: `cd mcp_server_dart && flutter test && cd ../flutter_mcp_toolkit_capability_core && dart test`
Expected: all pass.
...
Run: `grep -rn "'core_" --include="*.dart" mcp_server_dart/test flutter_mcp_toolkit_capability_core/test flutter_mcp_toolkit_capability_kernel/test 2>/dev/null`
...
cd ../flutter_mcp_toolkit_capability_core && dart test
cd ../flutter_mcp_toolkit_capability_kernel && dart test
cd ../flutter_mcp_toolkit_core && dart test
...
Run: `grep -rn "core_[a-z]" --include="*.dart" --include="*.txt" --include="*.mdx" mcp_server_dart flutter_mcp_toolkit_capability_core flutter_mcp_toolkit_capability_kernel tool docs 2>/dev/null | grep -v "core_capability"`
