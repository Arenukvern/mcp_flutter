...
| --------------------------------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`wait_for`** (text/element/time) | ✅ `wait_for` *(P0, shipped 2026-04-27)*        | Predicate kinds: text/noText/time/stable. Frame-counted under FakeAsync. Returns fresh snapshot in payload to avoid follow-up round-trip.                                                                  |
| `network_requests`                | **❌ missing**                                  | No HTTP traffic introspection. Cannot verify "did login POST happen". Needs toolkit-side `HttpOverrides` hook + ring buffer. **Architecturally heavy** — its own sub-plan. P3.                              |
| `press_key`                       | ✅ `press_key` *(P1, shipped 2026-04-27)*       | Two-pass dispatch: `HardwareKeyboard.handleKeyEvent` + direct `keyMessageHandler` invocation. Limitation documented: `TextField.onSubmitted` unreachable (textinput channel, not key events).               |
| `handle_dialog`                   | ✅ `handle_dialog` *(P1, dismiss only)*         | `accept` was dropped — post-`wait_for` it'd be a thin wrapper saving zero round-trips. `dismiss` calls `Navigator.pop` on the registered navigator key.                                                     |
| `navigate` (push/pop named route) | ✅ `navigate` *(P1, push/pop/popUntil)*         | Requires app to register a `GlobalKey<NavigatorState>` via `MCPToolkitBinding.setNavigatorKey`. `unawaited(pushNamed)` workaround for the deadlock when awaiting (Future resolves on pop, not display).      |
| `fill_form`                       | ✅ `fill_form` *(P2, shipped 2026-04-27)*       | Server-side loop over `_enterText`; no new toolkit code, no new wire RPC. Stops on first failure (transport OR toolkit-side `success/ok: false`).                                                          |
...
| `hover`                           | ✅ `hover` *(P2, shipped 2026-04-27)*           | Three-event sequence: `PointerAddedEvent` → off-screen hover → target hover. Drives `MouseRegion.onEnter`. Desktop/web only. DPR coord bug fixed 2026-04-28 — `resolveCenter` / `resolveBounds` now return logical coords on any DPR (`todo/dpr_resolve_center_bounds.md`). |
